            ...
            using (var graphics = Graphics.FromImage(result))
            {
                ...
                using (var geoEntitiesContext = new geoEntities())
                {
                    ...
                    var rect = new RectangleF(
                        graphics.VisibleClipBounds.X + padding.Left,
                        graphics.VisibleClipBounds.Y + padding.Top,
                        graphics.VisibleClipBounds.Width - padding.Right - padding.Left,
                        graphics.VisibleClipBounds.Height - padding.Bottom - padding.Top);
                    ...
                    for (var i = 0; i < allColors.Count; i++)
                    {
                        ...
                        graphics.DrawString(
                            ...
                            new SolidBrush(legendFontColor),
                            padding.Left,
                            padding.Top + i * height + captionSize.Height * lineSpacing);
                        graphics.FillRectangle(
                            new SolidBrush(color.Key),
                            padding.Left + maxLegendItemWidth + 5,
                            padding.Top + i * height + captionSize.Height * lineSpacing,
                            50,
                            height);
                        ;
